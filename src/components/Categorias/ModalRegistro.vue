    ...
    <div v-if="mostrar" class="fixed top-0 end-0 flex-col flex bg-white border-s w-[700px] max-w-[90%] h-full border-slate-300 text-slate-600 z-100">
      <div class="flex items-center justify-between w-full gap-3 p-3 border-b border-b-slate-300">
        <div class="flex items-center gap-3">
          ...
          <span class="font-bold text-md">Registrar categoría</span>
        </div>
      ...
      <form @submit.prevent="registrar" class="flex flex-col overflow-hidden text-sm h-full">
        ...
        <div class="flex items-center justify-end w-full gap-5 p-3 text-sm border-t border-slate-300 mt-auto">
          ...
          <button v-if="pagina === 2" type="submit" class="px-4 duration-300 bg-indigo-400 rounded-lg cursor-pointer text-indigo-50 hover:bg-indigo-300 h-9 active:scale-95">Registrar</button>
        </div>
      </form>
    </div>
  </transition>
...
const pagina = ref(1);
const categoria = ref({ nombre: '', icono: '', color: '', descripcion: '' });
const errores = ref({});
...
function cerrarModal() {
  ...
  mostrar.value = false;
  categoria.value = { nombre: '', icono: '', color: '', descripcion: '' };
  errores.value = {};
  pagina.value = 1;
}
...
function registrar() {
  if (validarFormulario(categoria.value, errores.value)) {
    emits('registrar', categoria.value);
    cerrarModal();
  }
}
