    ...
    <div v-if="mostrar" class="fixed top-0 end-0 flex-col flex bg-white border-s w-[700px] max-w-[90%] h-full border-slate-300 text-slate-600 z-120">
      <div class="flex items-center justify-between w-full gap-3 p-3 border-b border-b-slate-300">
        <div class="flex items-center gap-3">
          ...
          <span class="font-bold text-md">Regitrar producto</span>
        </div>
      ...
      <form @submit.prevent="actualizar" class="flex flex-col justify-between h-full overflow-hidden text-sm">
        ...
        <div class="flex items-center justify-end w-full gap-5 p-3 text-sm border-t border-slate-300">
          <button @click="cerrarModal" type="button" class="px-4 duration-300 bg-white border rounded-lg cursor-pointer border-slate-300 hover:opacity-50 text-slate-600 h-9 active:scale-95">Cancelar</button>
          <button type="submit" class="px-4 duration-300 bg-indigo-400 rounded-lg cursor-pointer text-indigo-50 hover:bg-indigo-300 h-9 active:scale-95">Actualizar</button>
        </div>
      </form>
    </div>
  </transition>
...
const actualizar = () => {
  if(validarFormulario(props.producto, errores.value)) {
    emits('actualizar', props.producto);
    cerrarModal();
  }
}
